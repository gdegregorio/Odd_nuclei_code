...
!      open(44,file='symV.dat',status='unknown',form='formatted')

!       do ii=1,dimbas
!        do jj=1,dimbas
!         if(dabs(Vcall(ii,jj)-Vcall(jj,ii)).gt.0.000000001) then
!          write(44,*) ii,jj,dabs(Vcall(ii,jj)-Vcall(jj,ii))        
!         end if
!        end do
!       end do
!       write(44,*) 'symmetricV'
...
!***************************************************************
       ...
       if (ii.eq.jj) en=levn(bas(ii)%ir)%ei+ph(bas(ii)%ilam)%el
       if(ii.ne.jj)  en=0.d0
       Amat(ii,jj)=en
       ser7(ii)=en
       !end if
       !if(ii.ne.jj) then
       !Amat(ii,jj)=Vcall(ii,jj)
       !end if
